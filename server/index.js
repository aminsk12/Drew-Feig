...
// middleware
const corsOptions = {
  origin: ['http://localhost:5173', 'http://localhost:5174', 'https://complit-stayvista.firebaseapp.com'],
  credentials: true,
...
async function run() {
  try {
    const usersCollection = client.db('aminVistaDb').collection('users')
    const roomsCollection = client.db('aminVistaDb').collection('rooms')
    const bookingsCollection = client.db('aminVistaDb').collection('bookings')
